...
const clamp255 = (x) => Math.min(255, Math.max(0, x));
/* ---------- sRGB gamma <-> linear ---------- */
...
export function linToSrgb(c) {
    return clamp255((c <= 0.0031308 ? 12.92 * c : 1.055 * Math.pow(c, 1 / 2.4) - 0.055) * 255);
}
...
export function parseCssColor(input) {
    ...
    if (m) {
        const ch = (x) => x.includes("%") ? clamp255(255 * pct(x)) : clamp255(num(x));
...
/* ---------- Helpers ---------- */
export function isOpaque(c) { return c.a >= 0.999; }
