...
describe('CLI basic commands', () => {
  ...
  it('validate fail-on error exits non-zero', () => {
    ...
    try { stdout = run('npx tsx ./cli/larissa.ts validate --fail-on error --summary json'); }
  ...
  it('validate fail-on off exits zero', () => {
    let code = 0; let stdout = '';
    try { stdout = run('npx tsx ./cli/larissa.ts validate --fail-on off --summary json'); }
    catch (e: any) { code = e.status || e.code || 1; stdout = e.stdout?.toString() || ''; }
    expect(code).toBe(0);
    const lines = stdout.trim().split(/\n+/);
    let parsed: any = null;
  ...
  it('set unknown id suggests alternatives and exits non-zero', () => {
    ...
    try { run('npx tsx ./cli/larissa.ts set color.palette.brand.999=#fff --dry-run --quiet'); }
